    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] shadow-xl overflow-hidden flex flex-col transition-all duration-300">
      <div className="p-6 sm:p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
        <div className="flex items-center gap-4">
           ...
           <div>
             <h2 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Chiến Thuật Hôm Nay</h2>
             <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400">Thứ tự ưu tiên dựa trên Deadline & Độ khó</p>
           </div>
        </div>
      </div>

      <div className="p-4 sm:p-8">
        {/* Responsive margin logic for the timeline line */}
        <div className="relative border-l-2 border-slate-200 dark:border-slate-700 ml-2 sm:ml-4 space-y-6 sm:space-y-8">
          {sortedTasks.map((task, index) => {
            ...
            return (
              <div key={task.id} className="relative pl-6 sm:pl-10 group">
                {/* Timeline Dot - Responsive Position */}
                <div 
                  className={`absolute -left-[7px] sm:-left-[9px] top-6 w-4 h-4 sm:w-5 sm:h-5 rounded-full border-4 transition-colors ${isFirst ? 'bg-white dark:bg-slate-800 scale-110' : 'bg-slate-100 dark:bg-slate-700 border-slate-200 dark:border-slate-600'}`}
                ...
                <div className={`p-4 sm:p-6 rounded-2xl border transition-all duration-300 hover:shadow-md ${isFirst ? 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 shadow-sm' : 'bg-slate-50/50 dark:bg-slate-900/50 border-transparent hover:border-slate-200 dark:hover:border-slate-800'}`}>
                  
                  {/* Flex container switches direction on mobile */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex-grow min-w-0">
                      {isFirst && (
                        <span className="inline-block px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider mb-2 sm:mb-3 bg-rose-100 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400">
                          Ưu tiên cao nhất
                        </span>
                      )}
                      <h4 className="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2 sm:gap-3 truncate">
                        <span className="text-xl sm:text-2xl">{task.icon}</span>
                        <span className="truncate">{task.subject}</span>
                      </h4>
                      <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 sm:mt-2 line-clamp-1">{task.description}</p>
                      
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-3 sm:mt-4">
                         <span className={`text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg whitespace-nowrap ${sessionInfo.color}`}>
                           {sessionInfo.type}
                         </span>
                         ...
                         {editingTaskId === task.id ? (
                            <div className="flex items-center gap-1 bg-white dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
                               <input 
                                 ...
                                 placeholder="vd: 45m"
                                 className="w-16 sm:w-20 px-1 sm:px-2 py-1 text-xs bg-transparent outline-none font-bold"
                               />
                            ...
                            <div 
                              className="flex items-center gap-1.5 group/edit cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg transition-colors bg-slate-100 dark:bg-slate-800" 
                         ...
                         <span className="text-[10px] sm:text-xs font-medium text-slate-400 ml-1 sm:ml-2">
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto flex-shrink-0 px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl font-bold text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all active:scale-95 flex items-center justify-center gap-2"
